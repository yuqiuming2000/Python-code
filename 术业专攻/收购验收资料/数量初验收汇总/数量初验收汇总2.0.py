...
new_wb=load_workbook('术业专攻\收购验收资料\数量初验收汇总\数量初验收模板.xlsx')
f_ws=new_wb.copy_worksheet(new_wb['模板'])
f_ws.title='汇总'
for file in files:
    file_path=path+file
    new_file_path=new_path+'数量初验收汇总表.xlsx'
...
#以上部分复制正文内容
                    ...
                    if r>7:
                        ...
                        if Difference>1:
                            ...
                            new_ws['{}'.format('P'+str(r+2))].value=sum
                        else:
                            new_ws['{}'.format('C'+str(r))].value='合计'
                            new_ws['{}'.format('P'+str(r))].value=sum
                    #以上内容判断空行的位置，填写合计和求和    
                    break
