...
import csv
with open('当当网2021年6月份销售排行榜.csv','w',newline='')as f:
    writer=csv.writer(f)
...
for i in range(1,11):
    ...
    for data in  datas:
        try:
            ...
            with open('当当网2021年6月份销售排行榜.csv','a',newline='',encoding='gb2312')as f:
                writer=csv.writer(f)
                row=[num,name,title,href,price_n,price_r,price_s]
                writer.writerow(row)
        except Exception:
            pass